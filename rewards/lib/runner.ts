...
export class Runner<Api extends AnyApi, ApiType extends ApiTypes, ApiAt> {
  ...
  private getApi(network: Networks) {
    if (this.#withApiRx) {
  ...
  private async toPromise<A, B>(x: Observable<A> | Promise<B>) {
    if (this.#withApiRx) {
  ...
  private async getTime(api: AnyApi, hash: string) {
    const apiAt = await api.at(hash)
    const time = await this.toPromise(apiAt.query.timestamp.now())
    return Number(time.toString());
  }

  private async getHash(api: AnyApi, blockNo: number) {
    const hash = await this.toPromise(api.rpc.chain.getBlockHash(blockNo))
    return hash.toString()
  }

  private async parseAt(api: AnyApi, at: number | string | undefined): Promise<number | 'now'> {
    switch (at) {
      case undefined:
      case null:
      case 'now':
  ...
  private async runOne({ network, at }: RunOptions, fn: (c: Context<Api, ApiAt>) => Promise<any> | void) {
  ...
  private async _run(fn: (c: Context<Api, ApiAt>) => Promise<any> | void) {
    ...
    if (argv.all) {
      ...
      const _networks = getNetworks(argv.network)
